...
const getStatusTagType = (status: HashCrackTaskStatus) => {
  switch (status) {
...
const getStatusText = (status: HashCrackTaskStatus) => {
  switch (status) {
    case HashCrackTaskStatus.ERROR:
      return () => t('error')
    case HashCrackTaskStatus.READY:
  ...
  <n-descriptions size="large" label-placement="left" separator="">
    <n-descriptions-item>
      ...
      <div class="found-word-list">
        <n-tag :type="getStatusTagType(status)" round :bordered="false">
